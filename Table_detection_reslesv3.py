...
class table_check(object):
    ...
    def new_lins_np(self, thled1, open_row, open_clo, line=5):
        ...
        img = cv.drawContours(image_longcont, [cont.T for cont in conts], -1, 255, 3)
        cv2.imshow('image_longcont', img)
    ...
    def find_long_lines(self, image):
        ...
        for cont in contours:
            ...
            if max_y >= maxh - 15 and min_y <= 15: # 意义是什么？？
                conts.append(cont_t)
        return conts
    ...
    def Cell_refactor(self, lines_row, lines_clo):
        contours_row, _ = cv2.findContours(lines_row, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        contours_clo, _ = cv2.findContours(lines_clo, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
            ...
            for row_w_ke, row_va in row_lines.items():
                ...
                if abs(clo_w_ke - row_w_ke) < 200:  # 竖线之间的距离与横线的长度容错小于100
                    ...
                    for row_enpoints in row_va[0]:
                        ...
                        if (abs(row_y - clo_max_y) < 200 and abs(row_y - clo_max_y1) < 200) or (
                                abs(row_y - clo_min_y) < 200 and abs(row_y - clo_min_y1) < 200):
    ...
    def class_tabel(self, lin_image, long_map=None, state='clo'):
        ...
        """
        contours, hierarchy = cv2.findContours(lin_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        image_zero_clo = np.zeros(lin_image.shape, np.uint8)
        image_zero_row = np.zeros(lin_image.shape, np.uint8)
        cont_class_lis = []
        cont_class_dict = {}
        ...
        if state == 'clo':
            ...
            for i in contours:
                ...
                if np.max(cont_T[0]) >= lin_image.shape[1] - 10 or np.min(cont_T[1]) <= 10: continue
    ...
    def table_detection_test(self, img):
        ...
        self.w_ro = self.width / self.new_width
        self.h_ro = self.height / self.new_height
        image = cv2.resize(img, (self.new_width, self.new_height)) # 图片放大到指定大小
        ...
        # titles = ['Thled1 Image', 'dilate', 'erode']
        # images = [thled1, dilate_row, erode]
        # for i in range(3):
        ...
        ####### numpy运算检测
        rows, clos = self.new_lins_np(thled1, row_image, clo_image, line=10) # 这里的thled1是经过膨胀腐蚀之后的表格图片
        if self.not_line:
            print("开始无线表检测")
            # demo1(img)
    ...
    def open_demo(self, binary):
        ...
        cv.imwrite("ima_test/open_r.jpg", open_r)
    ...
    def image_test(self, boxs, image, state='boxs', name=None):
        ...
        if state == 'boxs':
            ...
            # cv.imshow("box", image)
            # if name != None:
            #     cv2.imwrite("test_img/box_%s"%name,image)
        else:
            # 画表格外轮廓
            for ke, va in boxs.items():
                i = va
        ...
        if len(self.table_enpoint) != 0:
            ...
            if name != None:
                ...
                cv2.imwrite("test_img/table_%s" % current_time + ".jpg"
                                                                 "", image_s1)
        if len(self.table_enpoint) != 0:
    ...
    def get_angle(self):
        ...
        for box_ in demo2(2):  ### 1:住院票据  2:门诊票据
            ...
            cv2.putText(image, str(new_box["key_name"]), (int(new_box["x"] - 10), int(new_box["y"] - 10)),
